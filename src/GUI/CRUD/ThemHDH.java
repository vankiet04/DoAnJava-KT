...
public class ThemHDH extends javax.swing.JDialog {
    ...
    private ArrayList<DTO_HeDieuHanh> listHDHH = new ArrayList<>();
    public DAO_HeDieuHanh hdhh = new DAO_HeDieuHanh();

    public ThemHDH(java.awt.Frame parent, boolean modal, GUI.CRUD.QuanLyHDH qlhdh) {
        super(parent, modal);
        this.qlhdh = qlhdh;
        initComponents();
        listHDHH = hdhh.getAllData();
    }
    ...
    private void jButton1MousePressed(java.awt.event.MouseEvent evt) {                                      
        ...
        // Gọi hàm validateHDH để kiểm tra tính hợp lệ
        if (!validateHDH(hdhStr)) {
            return; // Nếu không hợp lệ, thoát khỏi hàm
        }

        // Debug: In ra danh sách tên hệ điều hành hiện tại
        for (DTO_HeDieuHanh dto : listHDH) {
            System.out.println("Tên trong danh sách: " + dto.getTenhdh());
        }

        // Thêm hệ điều hành vào danh sách
        DTO_HeDieuHanh hdh = new DTO_HeDieuHanh(bushdh.getMaxIDHDH() + 1, hdhStr);
        bushdh.insert(hdh);
    ...
    private boolean validateHDH(String hdhStr) {
        if (hdhStr.isEmpty()) {
            JOptionPane.showMessageDialog(null, "Tên hệ điều hành không được để trống");
            return false;
        } else if (!hdhStr.matches("[a-zA-Z\\s]+")) {
            JOptionPane.showMessageDialog(null, "Tên hệ điều hành chỉ chứa chữ cái");
            return false;
        }
        int i = 0;
        while (i < listHDHH.size()) {

            DTO_HeDieuHanh dto = listHDHH.get(i);
            if (dto.getTenhdh().equals(hdhStr)) {
