...
public class SuaNhaCungCapDialog extends javax.swing.JDialog {
    ...
    public boolean validateNhaCungCap(String ten, String diaChi, String email, String soDienThoai) {
        ...
        // Kiểm tra tên: cho phép chữ cái, số và dấu tiếng Việt, nhưng không cho ký tự đặc biệt
        if (!ten.matches("[a-zA-Z0-9\\p{L}\\s]+")) {
            JOptionPane.showMessageDialog(null, "Tên không đúng định dạng");
            return false;
        }

        // Kiểm tra số điện thoại: phải là số, đúng 10 chữ số, không chứa khoảng trắng
        if (!soDienThoai.matches("\\d{10}")) {
            JOptionPane.showMessageDialog(null, "Số điện thoại phải là số và có đúng 10 chữ số");
            return false;
        }

        // Kiểm tra địa chỉ: không chứa ký tự đặc biệt, chỉ cho phép chữ cái, số và dấu cách
        ...
        if (ten.equals(nccDTO.getTenncc()) && email.equals(nccDTO.getEmail()) && diaChi.equals(nccDTO.getDiachi()) && soDienThoai.equals(nccDTO.getsodienthoai())) {
            JOptionPane.showMessageDialog(null, "Không có gì thay đổi");
            return false;
        }

        // Kiểm tra trùng tên và số điện thoại với nhà cung cấp khác
        ArrayList<DTO_NhaCungCap> listnv = busncc.getAllData();
        for (DTO_NhaCungCap nv : listnv) {
            if (nv.getTenncc().equals(ten) && nv.getMancc() != nccDTO.getMancc()) {
        ...
        // Kiểm tra độ dài tên và địa chỉ
        if (ten.length() < 3) {
